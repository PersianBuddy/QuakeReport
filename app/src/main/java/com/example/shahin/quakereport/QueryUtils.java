...
import static com.example.shahin.quakereport.EarthquakeActivity.LOG_TAG;
...
public final class QueryUtils {
    ...
    //Method to fetch Earthquake from string url
    public static ArrayList<Earthquake> fetchEarthquake(String sampleUrl){
        //make a delay to simulate bad connection
        try {
            Thread.sleep(2000);
        ...
        //String response to http request
        String jsonResponse ="";
        try {
    ...
    //http request from the server to return json file as string variable
    private static String makeHttpRequest(URL url) throws IOException{
        ...
        try {
            ...
            if (connection.getResponseCode()==200){
                ...
                Log.e("Connection Failed","Connection Response Massage: "+connection.getResponseMessage());
            }
        }catch (IOException e){
            //Handle error
        }finally {
            if (connection!= null)connection.disconnect();
            //here java.io.IOException may happen
            if (inputStream!=null)inputStream.close();
        }
        return jsonResponse;
    }

    //a method to get a string and create a URL if it's possible
    private static URL createUrl(String sampleQuery){
        URL url=null;
        try {
            url= new URL(sampleQuery);
    ...
    //read from inputStream and return json file as String
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            //Create a date format for getting date
            DateFormat dateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM);
            //Create a date format to get time as clock
            ...
            for (int i=0;i<featureArray.length();i++){
                propertyObj =featureArray.getJSONObject(i).getJSONObject("properties");
                magTemp = propertyObj.getDouble("mag");
                locationTemp = propertyObj.getString("place");
                timeTemp =propertyObj.getLong("time");
                url = propertyObj.getString("url");
                dateTemp = new Date(timeTemp);
                Log.v("Array Values",magTemp + " " + locationTemp +""+ timeTemp);
                earthquakes.add(new Earthquake(locationTemp,dateFormat.format(dateTemp),magTemp,clockFormat.format(timeTemp),url));
            }
